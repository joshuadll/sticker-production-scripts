...
- aiUtils.jsx: NAME_REGEX, parseLayerName, mmToPoints, boundsCenter, isCaption,
  ...
  syncSpacingBuffer (per-element live 2mm keep-out halo named "{name} buffer" = the cutline
    duplicated INTO a dedicated "Spacing Buffer" sublayer at the TOP of the Cutlines layer (NOT a
    child of the cutline group — moved 2026-07-15 per artist feedback so ONE Layers-panel eyeball
    hides/shows every halo at once during hand-nesting), then rendered as a thin magenta/violet
    Multiply BAND (NOT a fill — a fill tinted the whole sticker pink; the band sits just OUTSIDE
    the cut so the art's true colours show) via a STROKE of width H + a LIVE Adobe Offset Path
    effect of +H/2, H = HALF the min spacing → band spans the cut line to +H. Magenta is the green
    Color Block's complement so it reads there (a cyan just muddies in). Drag-time aid for the 2mm
    rule: two pieces' halos meeting = exactly 2mm, OVERLAPPING halos darken = under spec —
    Illustrator has no live collision test, the darkening IS the signal. The sublayer is kept
    UNLOCKED + visible so a marquee/shift-click over a piece still grabs its halo — Illustrator
    selection is CROSS-LAYER, so the halo rides the artist's manual drag/scale just like the art
    (Sticker layer) does, even though it is no longer a group child (locking would drop it from the
    selection). Stays a true 1mm under resize ONLY with "Scale Strokes & Effects" OFF (set
    ...
    spacing/margin QA. Isolated from every consumer: Step 8c AND StepQA SKIP the "Spacing Buffer"
    sublayer by name (else the outward-offset halos read as cutlines → false failures / inflated
    occupancy), StepQA also keeps its " buffer" name-guard. removeAllSpacingBuffers drops the whole
    sublayer before export. NOT yet PS/AI-validated — the cross-layer drag-tracking especially
    needs an in-Illustrator check),
  removeAllSpacingBuffers (removes the whole "Spacing Buffer" sublayer from Cutlines before export),
